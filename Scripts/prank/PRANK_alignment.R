#title: PRANK_alignment
#author: Peter Fields
#date: 09 11 2021
#description: PRANK alignment script to be called from command line as follows (must have prank and parallel installed):

#conda activate prank
#ls *.fasta | parallel -j 1 Rscript PRANK_alignment.R
...
require(seqinr) #package for read/write fasta, version 3.6-1

paste("xinb3/",name_stem,".fasta",sep="")->m_nam
paste("iinb1/",name_stem,".fasta",sep="")->s_nam
...
BestFrame<-function(DNA){
        ...
        for (i in 1:6){ #for each frame
                ...
                for (j in 1:length(tmp)){ #for each peptide
                        if((nchar(tmp[j])>=pepl)&(substr(pep,1,1)=="M")){ #if its as long or longer than our longest and starts with an M, keep it
							...
							fr<-i #record frame
                        }else{ #otherwise, if it's longer (regardless of start codon) keep it
							if(nchar(tmp[j])>pepl){
								pep<-tmp[j]
								pepl<-nchar(pep)
								fr<-i
		...
		if(nchar(dns[[fr]])%%3==0){
			return(dns[[fr]]) #return the padded DNA in the best frame
		}
		if(nchar(dns[[fr]])%%3==1){
			return(paste(dns[[fr]],"-","-",sep="") )#return the padded DNA in the best frame
		}
		if(nchar(dns[[fr]])%%3==2){
			return(paste(dns[[fr]],"-",sep="") )#return the padded DNA in the best frame
		}		
...
cat(paste(">",name_stem,"_xinb3",sep=""),"\n",file=tname,append=FALSE)
cat(bmg,"\n",file=tname,append=TRUE)
cat(paste(">",name_stem,"_iinb1",sep="","\n"),file=tname,append=TRUE)
...
#now we need to identify which sites are represented in the dn/ds estimation
...
if(gsub("-","",bmg)==toupper(paste(mg,collapse=""))){ #if it wasn't turned around when finding the best frame
	...
	paste(mg,collapse="")->mag_full #this is the full D. magna sequence we read in, now we need the locations of the 'chunk' substrings
